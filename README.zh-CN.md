...
- WebDAV Class 1/2，`/` 与 `/webdav/` 均可（可关闭，不影响网页端）
...
3. 将 R2 bucket 绑定到 `BUCKET`。在 Pages → Settings → Variables and Secrets 里把 `WEBDAV_USERNAME` 和 `WEBDAV_PASSWORD` 加为 **Secret**（密钥，不要选 Text），然后重新部署。
