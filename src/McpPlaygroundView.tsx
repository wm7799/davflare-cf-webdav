...
function McpPlaygroundView({
  ...
  const [guideStatus, setGuideStatus] = useState<StepStatus>("idle");
  ...
  const loadGuide = useCallback(async () => {
    ...
    try {
      const text = await mcpInstructions(key);
      setGuideText(text);
      setGuideStatus(text ? "green" : "red");
    } catch (err) {
      setGuideText("");
      setGuideStatus("red");
      onNotify(errorMessage(err) || strings.mcpPlayGuideFetchFailed, "error");
    }
  }, [apiKey, onNotify]);
  ...
  const copyGuide = async () => {
    try {
      ...
      onNotify(strings.mcpPlayGuideCopied, "success");
    } catch {
      onNotify(strings.setupCopyFailed, "error");
    }
  };
        ...
        >
          ...
          >
            <Stack direction="row" alignItems="center" spacing={1}>
              {statusIcon(guideStatus)}
              <Typography sx={{ fontWeight: 700 }}>
                {strings.mcpPlayGuideTitle}
              </Typography>
            </Stack>
            <Stack direction="row" spacing={1}>
              <Button
                ...
                disabled={guideStatus === "loading" || !apiKey.trim()}
              >
                {strings.mcpPlayGuideFetch}
              </Button>
              {guideText && (
                <Button
                  size="small"
                ...
                >
                  {strings.mcpPlayGuideAction}
                </Button>
              )}
            </Stack>
          </Stack>
          <Typography variant="body2" color="text.secondary" sx={{ mb: 1 }}>
            {strings.mcpPlayGuideHint}
          </Typography>
          <Typography variant="caption" color="text.secondary" sx={{ display: "block", mb: 1 }}>
            {strings.mcpPlayToolCount}
          </Typography>
          {guideText && (
            <Box
              component="pre"
