...
describe("mcp protocol", () => {
  ...
  describe("self-describing surface (instructions + annotations)", () => {
    test("initialize ships non-empty instructions", async () => {
      const result = await dispatchMcpRequest(
        rpc({ method: "initialize", params: { protocolVersion: MCP_PROTOCOL_VERSION } }),
        mockApis()
      );
      expect(result.kind).toBe("rpc");
      if (result.kind !== "rpc") return;
      const body = result.body.result as { instructions?: string };
      expect(typeof body.instructions).toBe("string");
      expect(body.instructions).toBe(MCP_INSTRUCTIONS);
      expect(body.instructions!.length).toBeGreaterThan(300);
    });

    test("instructions cover the knowledge an agent needs", () => {
      // 这些是「不看文档也能操作」的最小必要知识，缺一条就会让调用方踩坑。
      expect(MCP_INSTRUCTIONS).toMatch(/object key/);
      expect(MCP_INSTRUCTIONS).toMatch(/path/);
      expect(MCP_INSTRUCTIONS).toMatch(/name/);
      expect(MCP_INSTRUCTIONS).toMatch(/1 MiB/);
      expect(MCP_INSTRUCTIONS).toMatch(/25 MB/);
      expect(MCP_INSTRUCTIONS).toMatch(/404/);
      expect(MCP_INSTRUCTIONS).toMatch(/trash/);
      expect(MCP_INSTRUCTIONS).toMatch(/\$\{env:\.\.\.\}/);
    });

    test("every tool carries a Chinese title", () => {
      for (const tool of MCP_TOOLS) {
        const title = (tool as { title?: string }).title;
        expect(typeof title, `${tool.name} title`).toBe("string");
        expect(title!.length, `${tool.name} title non-empty`).toBeGreaterThan(0);
        expect(/[\u4e00-\u9fa5]/.test(title!), `${tool.name} title is Chinese`).toBe(true);
      }
    });
    ...
    test("every tool carries annotations", () => {
      for (const tool of MCP_TOOLS) {
        ...
        expect(annotations, `${tool.name} annotations`).toBeTruthy();
        expect(
          annotations!.readOnlyHint === true || annotations!.destructiveHint === true,
          `${tool.name} declares read-only or destructive`
        ).toBe(true);
      }
    });

    test("read tools are read-only and destructive tools are flagged", () => {
      const byName = new Map<string, { name: string; annotations?: Record<string, boolean> }>(
        MCP_TOOLS.map((t) => [
          t.name as string,
          t as { name: string; annotations?: Record<string, boolean> },
        ])
      );
      const readOnly = ["list", "download", "zip", "search", "stat", "share_list", "trash_list", "image_list"];
      for (const name of readOnly) {
        expect(byName.get(name)?.annotations?.readOnlyHint, `${name} readOnlyHint`).toBe(true);
      }
      const destructive = ["delete", "trash_empty", "move", "upload", "image_delete"];
      for (const name of destructive) {
        expect(byName.get(name)?.annotations?.destructiveHint, `${name} destructiveHint`).toBe(true);
      }
    });

    test("upload param error explains the path/name split", async () => {
      // 把完整路径塞进 path、漏掉 name —— 这是最容易踩的坑。
      const result = await callTool("upload", { path: "docs/a.txt", content: "hi" });
      expect(toolPayload(result).isError).toBe(true);
      const text = toolPayload(result).content[0].text;
      expect(text).toMatch(/name/);
      expect(text).toMatch(/TARGET FOLDER/);
      expect(text).toMatch(/Correct shape/);
      expect(text).toMatch(/"path":"docs"/);
    });

    test("upload missing content error is self-explanatory", async () => {
      const result = await callTool("upload", { path: "docs", name: "a.txt" });
      expect(toolPayload(result).isError).toBe(true);
      const text = toolPayload(result).content[0].text;
