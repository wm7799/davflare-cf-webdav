...
describe("webdav OPTIONS / redirect / auth", () => {
  test("root-mount OPTIONS is unauthenticated and advertises DAV class", async () => {
    // 客户端在挂载前会先探 DAV/Allow；回 401 会让 Windows 资源映射 /
    // Cyberduck 直接判定「不是 WebDAV 端点」。
    const bucket = new InMemoryBucket();
    const response = await call(
      req("/webdav/", "OPTIONS", { "X-Davflare-Mount": "root" }),
      makeEnv(bucket)
    );
  ...
  test("root-mount unauthenticated PROPFIND still returns 401 + WWW-Authenticate", async () => {
    const bucket = new InMemoryBucket();
    const response = await call(
      req("/webdav/", "PROPFIND", { "X-Davflare-Mount": "root", Depth: "1" }),
      makeEnv(bucket)
    );
  ...
  test("root-mount PROPFIND with valid credentials succeeds", async () => {
    const bucket = new InMemoryBucket();
    const response = await call(
      req("/webdav/", "PROPFIND", {
        "X-Davflare-Mount": "root",
        Authorization: AUTH,
        Depth: "1",
      }),
      makeEnv(bucket)
...
describe("webdav PROPFIND", () => {
  ...
  test("root-mounted WebDAV returns root-relative hrefs", async () => {
    const bucket = new InMemoryBucket();
    bucket.seedDir("docs");
    ...
    expect(xml).toContain("<href>/docs/</href>");
    expect(xml).toContain("<href>/docs/a.txt</href>");
    expect(xml).not.toContain("/webdav/");
  });
