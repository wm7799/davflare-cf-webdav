...
/**
 * 拉取 initialize 下发的 instructions —— 也就是「AI 连上后看到的使用说明」。
 * 网页侧把它原样展示出来，方便人确认这段自描述确实存在。
 */
export async function mcpInstructions(apiKey: string): Promise<string> {
  const body = await postMcp(
    ...
    "initialize",
    {
      ...
      capabilities: {},
      clientInfo: { name: "davflare-playground", version: "1.0.0" },
    },
    1
  );
  if (!isRecord(body) || !("result" in body) || !isRecord(body.result)) {
    throw new Error("Invalid initialize result");
  }
  const value = (body.result as Record<string, unknown>).instructions;
