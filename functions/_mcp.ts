 ...
 * 连上就能看懂的用法说明。
 *
 * MCP 规范里 `initialize` 的 `instructions` 会由客户端注入模型上下文，
 * 因此这里要写「不看文档也够用」的最小必要知识：心智模型、参数语义、
 * 容量边界、开关依赖、典型工作流。保持精简——它每次都占上下文。
 */
export const MCP_INSTRUCTIONS = [
  "# Davflare 网盘（对象存储 drive，key 是文件路径）",
  "",
  "这是一套文件网盘工具。所有路径都是 R2 的 object key，用 `/` 分层；空字符串 `\"\"` 表示根目录。",
  "没有真正的「文件夹」——目录靠 key 前缀表达，所以 `mkdir` 只是占位，`list` 会自动按前缀聚出目录。",
  "",
  "## 先看再动（重要）",
  "不要凭猜的路径直接调用 write 类工具。默认流程：",
  "1. 先 `list` 根目录（`path: \"\"`）看清现有结构；",
  "2. 需要定位具体文件用 `search`；",
  "3. 确认真实 key 后再 `upload` / `move` / `delete`。",
  "",
  "## 两个最容易搞错的参数",
  "- `path` 是**目标目录**，不是完整文件路径。",
  "- `name` 才是文件名，要单独传。",
  "  要放到 `docs/` 下叫 `a.txt`：传 `path: \"docs\"` + `name: \"a.txt\"`，**不要**写成 `path: \"docs/a.txt\"`。",
  "  （若把完整路径塞进 `path` 而漏了 `name`，会报 `name is required`。）",
  "",
  "## 容量边界（决定了你该用哪个工具）",
  "- `upload`：≤ 1 MiB 内联直传；1 MiB–25 MB 自动改走 multipart 分块，你不用管；> 25 MB 工具会报错，请改用网页端或 API key 脚本。",
  "- `download`：≤ 1 MiB 内联返回。更大的文件用 `part`（1 起）配合 `partSize`（默认 1 MiB）翻页取 base64。",
  "- `zip`：目录打包，同样 1 MiB 内联 / `part` 分页，硬上限 25 MB。",
  "- 读大文件时**务必翻页读全**，不要只拿第一页就当完整内容。",
  "",
  "## 删除是可撤销的",
  "`delete` 默认软删除进回收站；`trash_list` 查看、`trash_restore` 捞回、`trash_empty` 永久清空。",
  "只有明确要求「永久删除」时才传 `hard: true`。",
  "",
  "## 功能开关导致的 404",
  "以下工具依赖账号里对应的功能开关，关闭时会直接报错而不是静默失败：",
  "- `publish_site` / `sites_*` → 需要 Sites 功能；",
  "- `image_*` → 需要 Image Host 功能。",
  "遇到这类报错时，说明该功能未开启，不要去猜路径或重试。",
  "",
  "## `push` 的安全约束",
  "`push` 往 `agents/{global|agent|agent/project}/` 写 skill / rule / MCP 配置。",
  "其中 `mcp.json` **禁止明文密钥，只接受 `${env:...}` 占位符**，否则会被拒绝——这是刻意的安全设计。",
  "",
  "## 典型任务",
  "- 备份目录：`list` 递归读出 → 逐个 `download`；或直接 `zip` 一次拿完。",
  "- 建站：`mkdir` + `upload` 放文件 → `publish_site` 指定 slug 与源目录。",
  "- 分享文件：`share_create` 拿公开链接；`share_list` / `share_revoke` 管理。",
].join("\n");
...
/**
 * 参数错误尽量自解释：除了「缺什么」，还给出正确调用形状，
 * 让调用方能一次自我纠正，而不是靠反复试。
 */
function toolParamError(
  detail: string,
  toolName: string,
  ...
  return toolError(
    [
      ...
      "",
      `Correct shape: ${JSON.stringify({ name: toolName, arguments: example })}`,
    ].join("\n")
  );
}
...
async function callTool(
  ...
  switch (name) {
    ...
    case "upload": {
      ...
      if (!fileName) {
        // 最常见的误用：把完整路径塞进 path，漏了 name。
        return toolParamError(
          "`name` (file name) is required. Note `path` is the TARGET FOLDER; the file name goes in `name`.",
          "upload",
          { path: "docs", name: "a.txt", content: "hello" }
        );
      }
      if (!Object.prototype.hasOwnProperty.call(args, "content")) {
        return toolParamError(
          "`content` is required (file content; utf8 text or base64 per `encoding`).",
          "upload",
          { path: "docs", name: "a.txt", content: "hello" }
        );
      }
