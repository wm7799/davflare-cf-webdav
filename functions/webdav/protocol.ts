...
const ROOT_DAV_MOUNT_HEADER = "X-Davflare-Mount";
...
function getDavMount(request: Request): string {
  return request.headers.get(ROOT_DAV_MOUNT_HEADER) === "root" ? "" : DAV_ENDPOINT;
}
...
function getResourceHref(
  ...
  const root = mount === "" ? "/" : DAV_ENDPOINT_WITH_SLASH;
  if (key === "") {
    return root;
  }
  const encodedPath = key.split("/").map(encodeURIComponent).join("/");
  return `${mount === "" ? "/" : DAV_ENDPOINT_WITH_SLASH}${encodedPath}${isCollection ? "/" : ""}`;
}

function getRequestResourceHref(
  request: Request,
  key: string,
  isCollection: boolean,
): string {
  return getResourceHref(key, isCollection, getDavMount(request));
}
...
function parseDestinationPath(
  ...
  try {
    ...
    if (pathname === DAV_ENDPOINT || pathname.startsWith(`${DAV_ENDPOINT}/`)) {
      return decodeResourcePath(pathname.slice(DAV_ENDPOINT.length) || "/");
    }
    if (request?.headers.get(ROOT_DAV_MOUNT_HEADER) === "root") {
      return decodeResourcePath(pathname);
    }
    return null;
  } catch {
    return null;
...
function createdResponse(
  ...
  headers.set("Location", getResourceHref(resourcePath, isCollection, mount));
...
function fromR2Object(
  object: R2Object | DavObject | null | undefined,
  mount = DAV_ENDPOINT,
): DavProperties {
      ...
      lockDetails.length === 0
        ...
        : getLockDiscovery(
            lockDetails.map((lockDetail) => ({
              ...lockDetail,
               root: getResourceHref(object.key, isCollection, mount),
            })),
          ),
...
function getLivePropertyValue(
  ...
  if (property.namespaceURI === DAV_NAMESPACE) {
    return fromR2Object(object, mount)[property.localName as keyof DavProperties];
  }
  if (
...
async function handleGet({
  ...
  if (new URL(request.url).pathname.endsWith("/")) {
    ...
    let prefix = path;
    if (path !== "") {
      page += `<a href="${escapeXml(getRequestResourceHref(request, getParentPath(path), true))}">..</a><br>`;
      prefix = `${path}/`;
    }
    ...
    for await (const object of listAll(bucket, path === "" ? undefined : prefix)) {
      ...
      const href = getRequestResourceHref(request, object.key, object.isCollection === true);
      const name =
...
async function handlePut({
  ...
  return existing === null
    ? createdResponse(path, false, "", getDavMount(request))
...
async function handleMkcol({
  ...
  return createdResponse(path, true, "", getDavMount(request));
}
...
function generatePropfindResponse(
  ...
  const href =
    object === null
      ? (mount === "" ? "/" : DAV_ENDPOINT_WITH_SLASH)
      : getResourceHref(object.key, isCollectionObject(object), mount);
  const deadProperties = getDeadProperties(object?.customMetadata);
  const liveProperties = Object.entries(fromR2Object(object, mount)).flatMap(([key, value]) =>
  ...
  switch (propfindRequest.mode) {
    ...
    case "propname": {
      okProperties = [
        ...Object.entries(fromR2Object(object, mount)).flatMap(([key, value]) =>
    ...
    case "prop": {
      for (const property of propfindRequest.properties) {
        const liveValue = getLivePropertyValue(object, property, mount);
...
async function handlePropfind({
  ...
  const mount = getDavMount(request);
  let page = `<?xml version="1.0" encoding="utf-8"?>
<multistatus xmlns="DAV:" xmlns:fd="${FLAREDRIVE_NAMESPACE}">`;

  if (path === "") {
    page += generatePropfindResponse(null, propfindRequest, mount);
    isCollection = true;
  } else {
    ...
    switch (depth) {
      ...
      case "infinity": {
        ...
        for await (const object of listAll(bucket, prefix, depth === "infinity")) {
          page += generatePropfindResponse(object, propfindRequest, mount);
        }
        break;
      }
      default:
...
<multistatus xmlns="DAV:">
  <response>
    <href>${escapeXml(getRequestResourceHref(request, object.key, isCollection))}</href>`;
...
async function handleCopy({
  ...
  const destination = parseDestinationPath(destinationHeader, request.url, request);
  if (destination === null) {
    return new Response("Bad Request", { status: 400 });
  }
  ...
  return destinationExists
    ? new Response(null, { status: 204 })
    : createdResponse(destination, isDirectory, "", getDavMount(request));
}
...
async function deleteDestination(
  ...
  const destinationPath = destinationHeader
    ? parseDestinationPath(destinationHeader, sourceRequest.url, sourceRequest)
    : null;
  if (destinationHeader && destinationPath === null) {
    return new Response("Bad Request", { status: 400 });
  }
  const destinationUrl = new URL(sourceRequest.url);
  destinationUrl.pathname = `${getDavMount(sourceRequest) || ""}/${destinationPath ?? ""}`
    .replace(/\/+/g, "/");
...
async function handleLock({
  ...
  const lockDetails: LockDetails = {
    ...
    timeout,
    expiresAt,
    root: getRequestResourceHref(request, resource.key, isCollection),
  };
  const updatedLocks =
...
<prop xmlns="DAV:"><lockdiscovery>${getLockDiscovery(updatedLocks)}</lockdiscovery></prop>`,
    {
      ...
      headers: {
        ...
        ...(existingLock
          ? {}
          : { Location: getRequestResourceHref(request, resource.key, isCollection) }),
      },
    },
  );
}
...
async function handleRequest(context: PagesContext): Promise<Response> {
  ...
  // 根域名直挂（中间件打了 X-Davflare-Mount: root）时，OPTIONS 必须免认证：
  // 客户端在收发凭据前先探 DAV/Allow 头来判定「这是不是 WebDAV 端点」，
  // 回 401 会让 Windows 资源映射 / Cyberduck 等直接判定挂载失败。
  // OPTIONS 无副作用，放开不影响安全（路径仍是 /webdav/*，仅走根域名入口）。
  const isRootMount = request.headers.get(ROOT_DAV_MOUNT_HEADER) === "root";
  const skipAuth =
  ...
  // 根挂载下的未认证请求：返回 401 时必须带 WWW-Authenticate，
  // 否则客户端不会弹认证/不会补发凭据。
  if (!skipAuth && isRootMount) {
