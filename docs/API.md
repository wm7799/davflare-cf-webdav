...
#### Self-describing: usable on connect

The server is built so an MCP client can operate correctly **without reading docs first**, via three things:

1. **`initialize` returns `instructions`** (written in Chinese). Clients inject this into model context. It covers the mental model (paths are R2 object keys), the "`list` before you write" default flow, the `path` = target folder / `name` = file name split, the 1 MiB and 25 MB limits, `delete` defaulting to trash, feature-switch 404s, the `push` no-raw-secrets rule, and three typical tasks (backup / publish a site / share).
2. **Every tool carries a `title` (short Chinese name) and `annotations`.** `readOnlyHint: true` marks read-only tools (`list` / `download` / `search` / `stat` / `zip` / `*_list`); `destructiveHint: true` marks destructive ones (`delete` / `trash_empty` / `move` / `upload` / `image_delete`, etc.) so clients can add confirmation. All 25 tools are covered.
3. **Parameter errors are self-explanatory.** Errors state what is wrong *and* include a `Correct shape` example so the caller can self-correct in one step. Example — a full path mistakenly passed as `path`:

   ```json
   {"name":"upload","arguments":{"path":"docs","name":"a.txt","content":"hello"}}
   ```

The web playground at `#/mcp` has a "Fetch live instructions" button that shows and copies the exact `instructions` the server returns, so you can confirm the self-description is live.
